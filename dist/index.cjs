// Auto-generated CJS bundle for pkg packaging - DO NOT EDIT
...
var SessionManager = class _SessionManager {
  ...
  static REWIND_WINDOW_MS = 3e4;
  static REWIND_MIN_RACE_TIME = 5;
  static MIN_LAP_SECS = 20;
  ...
  noteTick(isRaceOn, currentLap, raceTime) {
    ...
    if (!isRaceOn || raceTime > 0 && raceTime + 0.25 < this.prevRaceTime) {
      this.rewindGuard = _SessionManager.REWIND_GUARD_TICKS;
    }
    if (raceTime > 0) {
      this.prevRaceTime = raceTime;
    }
    let completed = null;
    if (isRaceOn && this.rewindGuard === 0 && this.prevCurrentLap > _SessionManager.MIN_LAP_SECS && currentLap < 1) {
  ...
  checkReopen(newRaceTime, nowWallMs) {
    ...
    if (gapMs < _SessionManager.REWIND_WINDOW_MS && newRaceTime > _SessionManager.REWIND_MIN_RACE_TIME && newRaceTime < this.lastRaceTimeAtClose) {
...
var __dirname;
if (void 0) {
  __dirname = import_path.default.dirname((0, import_url.fileURLToPath)(void 0));
} else {
  __dirname = import_path.default.dirname(__filename);
}
var PORT = process.env.PORT || 20440;
var HTTP_PORT = process.env.HTTP_PORT || 3e3;
var OUTPUT_DIR = "./sessions";
var sessionManager = new SessionManager(true);
var currentSession = null;
var currentSessionPackets = [];
var currentSessionLaps = [];
var sessionCounter = 0;
var lastClosedSessionData = null;
var sseClients = /* @__PURE__ */ new Set();
var diagPacketsTotal = 0;
var diagParseErrors = 0;
var diagLastParseError = null;
var diagLastPacketMs = null;
var diagLastParseErrorMs = null;
var diagParseErrLogCooldown = 0;
function ensureOutputDir() {
  if (!import_fs.default.existsSync(OUTPUT_DIR)) {
    import_fs.default.mkdirSync(OUTPUT_DIR, { recursive: true });
  }
}
...
function buildCompactExport(sessionPackets, sessionLaps, bestLap, sessionInfo) {
  ...
  let maxSpeed = 0, maxRpm = 0, maxPower = 0, maxTorque = 0, maxBoost = -Infinity;
  let totalFuel = 0, fuelSamples = 0;
  let maxLatG = 0, maxLongG = 0;
  for (const p of pkts) {
  ...
  const summary = {
    ...
    durationMs: pkts.length > 0 ? pkts[pkts.length - 1].timestampMs - pkts[0].timestampMs : 0,
    packetCount: pkts.length,
    lapCount: sessionLaps.length,
    bestLap: bestLap > 0 ? Math.round(bestLap * 1e3) / 1e3 : null,
    maxSpeedKmh: Math.round(maxSpeed * 10) / 10,
    maxRpm: Math.round(maxRpm),
    maxPowerKw: Math.round(maxPower / 1e3 * 10) / 10,
    maxTorqueNm: Math.round(maxTorque * 10) / 10,
    maxBoostPsi: Math.round(maxBoost * 100) / 100,
    avgFuel: fuelSamples > 0 ? Math.round(totalFuel / fuelSamples * 1e3) / 1e3 : null,
  ...
  const lapStats = sessionLaps.map((lap) => {
    return {
      ...
      lapTime: Math.round(lap.lapTime * 1e3) / 1e3
    };
  });
  const sectors = [];
  ...
  for (let li = 0; li < lapBoundaries.length - 1; li++) {
    const start2 = lapBoundaries[li];
    const end = lapBoundaries[li + 1];
    const lapPkts = pkts.slice(start2, end);
    ...
    for (let s = 0; s < sectorsPerLap; s++) {
      ...
      let sumSpeed = 0, segMaxSpeed = 0, sumRpm = 0, segMaxRpm = 0;
      let sumThrottle = 0, sumBrake = 0;
      let sumLatG = 0, sumLongG = 0;
      let sumTempFl = 0, sumTempFr = 0, sumTempRl = 0, sumTempRr = 0;
      let sumBoost = 0, sumPower = 0;
      for (const p of seg) {
        sumSpeed += p.speedKmh;
      ...
      sectors.push({
        ...
        avgPowerKw: Math.round(sumPower / n / 1e3 * 10) / 10
      });
    }
  }
  ...
  for (let i = 0; i < pkts.length; i += sampleFactor) {
    ...
    samples.push({
      ...
      powerKw: Math.round(p.power / 1e3 * 10) / 10,
      ...
      fuel: Math.round(p.fuel * 1e3) / 1e3,
...
function calculateSessionStats(packets) {
  ...
  return {
    maxSpeedMs: Math.round(maxSpeed * 1e3) / 1e3,
...
function saveSession(session) {
  ...
  const filepath = import_path.default.join(OUTPUT_DIR, filename);
  const sessionData = {
    id: session.id,
  ...
  import_fs.default.writeFileSync(filepath, JSON.stringify(sessionData, null, 2));
  return filepath;
}
...
function closeSession() {
  ...
  if (filepath) {
    const savedData = JSON.parse(import_fs.default.readFileSync(filepath, "utf8"));
  ...
  return new Promise((resolve) => {
    ...
    server.on("message", (msg, rinfo) => {
      ...
      try {
        ...
        broadcastToSSE(json);
        const timedLap = pkt.currentLap > 0;
...
async function startHTTP() {
  const server = import_http.default.createServer((req, res) => {
    ...
    if (req.url === "/export" || req.url?.startsWith("/export?")) {
      const params = req.url.startsWith("/export?") ? new URLSearchParams(req.url.split("?")[1]) : null;
      ...
      if (currentSession) {
        ...
        const bestLap = sessionManager.bestForClose() > 0 ? sessionManager.bestForClose() : currentSession.bestLap;
        const pkts = dsFactor > 0 ? downsample(sessionPackets, dsFactor) : sessionPackets;
        exportData = {
    ...
    if (req.url === "/export-compact" || req.url?.startsWith("/export-compact?")) {
      const params = req.url.startsWith("/export-compact?") ? new URLSearchParams(req.url.split("?")[1]) : null;
      const dsFactor = params ? parseInt(params.get("downsample"), 10) || 30 : 30;
      ...
      if (currentSession) {
        ...
        if (compactData) {
          ...
          compactData.endedAt = currentSession.endedAt ? new Date(currentSession.endedAt).toISOString() : null;
        }
        exportId = currentSession.id;
      } else if (lastClosedSessionData) {
        ...
        JSON.stringify(
          {
            ...
            lastParseErrorAgoMs: diagLastParseErrorMs ? now - diagLastParseErrorMs : null,
    ...
    if (req.url === "/sessions") {
      ensureOutputDir();
      try {
        const files = import_fs.default.readdirSync(OUTPUT_DIR).filter((f) => f.startsWith("session_") && f.endsWith(".json")).sort().reverse();
        const list = files.map((f) => {
          const raw = JSON.parse(
            import_fs.default.readFileSync(import_path.default.join(OUTPUT_DIR, f), "utf8")
          );
          return {
            id: raw.id,
    ...
    if (req.url?.startsWith("/session?")) {
      ...
      ensureOutputDir();
      try {
        const files = import_fs.default.readdirSync(OUTPUT_DIR).filter(
          (f) => f.startsWith(`session_${String(sid).padStart(4, "0")}_`) && f.endsWith(".json")
        );
        if (files.length === 0) {
        ...
        const data = import_fs.default.readFileSync(import_path.default.join(OUTPUT_DIR, files[0]), "utf8");
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(data);
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
    ...
    if (req.url === "/" || req.url === "/index.html") {
      const htmlPath = import_path.default.join(__dirname, "..", "public", "index.html");
      if (import_fs.default.existsSync(htmlPath)) {
        res.writeHead(200, { "Content-Type": "text/html" });
        import_fs.default.createReadStream(htmlPath).pipe(res);
      } else {
        res.writeHead(200, { "Content-Type": "text/html" });
    ...
    if (req.url?.startsWith("/assets/")) {
      const safePath = import_path.default.join(__dirname, "..", decodeURIComponent(req.url));
      if (!safePath.startsWith(import_path.default.resolve(import_path.default.join(__dirname, "..")))) {
        res.writeHead(403);
        res.end("Forbidden");
        return;
      }
      if (import_fs.default.existsSync(safePath) && import_fs.default.statSync(safePath).isFile()) {
        const buf = import_fs.default.readFileSync(safePath);
        let ct = "application/octet-stream";
        if (buf[0] === 82 && buf[1] === 73 && buf.length > 11 && buf[8] === 87 && buf[9] === 69)
          ct = "image/webp";
        else if (buf[0] === 137 && buf[1] === 80) ct = "image/png";
        else if (buf[0] === 255 && buf[1] === 216) ct = "image/jpeg";
        else if (buf[0] === 71 && buf[1] === 73) ct = "image/gif";
        else if (import_path.default.extname(safePath).toLowerCase() === ".svg")
          ct = "image/svg+xml";
        res.writeHead(200, {
          "Content-Type": ct,
...
<script>
  ...
  function updateTire(barId, valId, temp) {
    ...
    if (val) {
      val.textContent = temp != null ? temp.toFixed(0) + '\xB0C' : '--';
  ...
  // full-resolution tile pixels (zoom 14, tile range 8128-8191 \xD7 256px).
...
async function start() {
  ...
  console.log(`[output] sessions will be saved to ${import_path.default.resolve(OUTPUT_DIR)}`);
